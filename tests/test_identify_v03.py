...
def test_contraction_reports_ratio_and_notidentified_boundary():
    ...
    claim = identify_from_contraction(diag, minimum_contraction=0.5, target="beta")
    assert claim.status is ClaimStatus.SUPPORTED

    weak = contraction_diagnostic(prior_sd=1.0, posterior_samples=(-0.9, 0.9, -0.8, 0.8))
    claim = identify_from_contraction(weak, minimum_contraction=0.5, target="beta")
    assert claim.status is ClaimStatus.NOT_IDENTIFIED
