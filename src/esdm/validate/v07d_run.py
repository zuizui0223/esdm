"""Equal-dimension static-versus-dynamic held-out comparison for v0.7d."""
...
def summarize_v07d(records: Sequence[V07DReplicate]) -> V07DSummary:
    rows = tuple(records)
    if not rows:
        raise ValueError("v0.7d records must be non-empty")
    n = len(rows)
