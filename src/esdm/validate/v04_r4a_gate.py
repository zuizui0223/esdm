"""Mechanical evaluator for the prospective v0.4-R4a qualification gate."""
...
class V04R4AQualificationSummary:
    ...
    r3_spatial_sequence_preserved: bool
    phase_balance_preserved: bool


@dataclass(frozen=True, slots=True)
class V04R4AGateCheck:
    name: str
    passed: bool
    observed: object
    criterion: str
...
@dataclass(frozen=True, slots=True)
class V04R4ADecision:
    passed: bool
    checks: tuple[V04R4AGateCheck, ...]


@dataclass(frozen=True, slots=True)
class V04R4AIdentificationResult:
    positive_structural_pass: bool
    positive_practical_pass: bool
    sparse_structural_pass: bool
    sparse_practical_refused: bool
...
def _check(name, passed, observed, criterion) -> V04R4AGateCheck:
    return V04R4AGateCheck(
        name=str(name),
        passed=bool(passed),
        observed=observed,
...
def evaluate_v04_r4a_qualification(
    summary: V04R4AQualificationSummary,
) -> V04R4ADecision:
    checks = (
        _check(
        ...
        _check(
            "r3_spatial_sequence_preserved",
            summary.r3_spatial_sequence_preserved,
            summary.r3_spatial_sequence_preserved,
            "is True",
        ),
        _check(
            "phase_balance_preserved",
            summary.phase_balance_preserved,
            summary.phase_balance_preserved,
            "is True",
        ),
    )
    return V04R4ADecision(
...
def evaluate_v04_r4a_identification(
    source_csv_text: str,
) -> V04R4AIdentificationResult:
    """Evaluate R4a positive design against unchanged R2 refusal controls."""

    positive = build_v04_r4a_fixture(source_csv_text)
    ...
    return V04R4AIdentificationResult(
...
def qualification_summary(
    source_csv_text: str,
    identification: V04R4AIdentificationResult,
) -> V04R4AQualificationSummary:
    fixture = build_v04_r4a_fixture(source_csv_text)
    r3 = build_v04_r3a_fixture(source_csv_text)
    ...
    return V04R4AQualificationSummary(
        ...
        annotated_space_count=len({key[0] for key in annotated_keys}),
        annotated_time_count=len({(key[1], key[2]) for key in annotated_keys}),
        ...
        r3_spatial_sequence_preserved=(
            fixture.annotated_spaces == r3.annotated_spaces
        ),
        phase_balance_preserved=(
            fixture.annotated_times == R4A_FROZEN_TEMPORAL_CONTEXTS
        ),
    )
