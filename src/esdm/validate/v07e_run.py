"""Reciprocal static-world predictive specificity benchmark for v0.7e."""
...
from .v07e_fixture import build_v07e_fixture
...
class V07ESummary:
    replicates: int
    fit_count: int
    static_better_rate: float
    mean_static_gain: float
    minimum_static_gain: float
...
def summarize_v07e(records: Sequence[V07EReplicate]) -> V07ESummary:
    rows = tuple(records)
    if not rows:
        raise ValueError("v0.7e records must be non-empty")
    n = len(rows)
    gains = tuple(row.static_gain for row in rows)
    return V07ESummary(
        replicates=n,
        fit_count=2 * n,
        static_better_rate=sum(gain > 0.0 for gain in gains) / n,
        mean_static_gain=math.fsum(gains) / n,
        minimum_static_gain=min(gains),
