"""Poisson occurrence records conditioned on marginal occupancy."""
...
class OccupiedPresenceOnly:
    """Presence-only records generated from intensity times marginal occupancy."""
    ...
    consumes: frozenset[str] = frozenset({"log_intensity", "occupancy"})
    required_latent_channels: frozenset[str] = frozenset(
        {"log_intensity", "occupancy"}
    )
    ...
    def expected_rates(
        ...
        *,
        ...
        if species not in fields.occupancy:
            raise ValueError(
                f"species {species!r} lacks a marginal occupancy latent field"
            )
    ...
    def expected_rate_array(
        ...
        *,
        ...
        if intensity.keys != occupancy.keys:
            raise ValueError("latent intensity/occupancy context orders must match")
        obs = {} if theta_obs is None else theta_obs
        covs = {} if covariates is None else covariates
        ...
        return ContextArray(
            intensity.keys,
            array_module.exp(intensity.values)
            * occupancy.values
            * effort
            * detection,
        )
    ...
    def validate_species_data(self, species, data, keys) -> None:
        ...
        if any(value < 0 for value in values):
            raise ValueError("occupied presence counts must be non-negative")
