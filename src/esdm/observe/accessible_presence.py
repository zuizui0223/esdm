"""Poisson occurrence records conditioned on ecological accessibility."""
...
class AccessiblePresenceOnly:
    """Presence-only records generated from suitability times accessibility."""
    ...
    consumes: frozenset[str] = frozenset(
        {"log_intensity", "log_accessibility"}
    )
    required_latent_channels: frozenset[str] = frozenset(
        {"log_intensity", "log_accessibility"}
    )
    ...
    def expected_rates(
        ...
        *,
        ...
        if species not in fields.log_accessibility:
            raise ValueError(
                f"species {species!r} lacks a log-accessibility latent field"
            )
        ...
        for key, log_ecological in fields.log_intensity[species].items():
            ...
            rates[key] = (
                exp_fn(log_ecological + fields.log_accessibility[species][key])
                * effort
                * detection
            )
        return rates
    ...
    def expected_rate_array(
        ...
        *,
        ...
        intensity = fields.log_intensity[species]
        accessibility = fields.log_accessibility[species]
        if intensity.keys != accessibility.keys:
            raise ValueError("latent intensity/accessibility context orders must match")
        obs = {} if theta_obs is None else theta_obs
        covs = {} if covariates is None else covariates
        ...
        return ContextArray(
            intensity.keys,
            array_module.exp(intensity.values + accessibility.values)
            * effort
            * detection,
        )
    ...
    def validate_species_data(self, species, data, keys) -> None:
        ...
        if any(value < 0 for value in values):
            raise ValueError("accessible presence counts must be non-negative")
