"""Direct Poisson calibration of latent accessibility."""
...
class AccessibilityCount:
    """Direct accessibility observations independent of habitat suitability intensity."""
    ...
    consumes: frozenset[str] = frozenset({"log_accessibility"})
    required_latent_channels: frozenset[str] = frozenset({"log_accessibility"})
    ...
    def expected_rates(
        ...
        *,
        ...
        if species not in fields.accessibility:
            raise ValueError(
                f"species {species!r} lacks an accessibility latent field"
            )
        obs = {} if theta_obs is None else theta_obs
        covs = {} if covariates is None else covariates
        detection = self._detection_value(obs)
        rates = {}
        for key, accessibility in fields.accessibility[species].items():
            effort = self.effort.at(
                key,
                theta=obs,
                covariates=covs,
            )
            rates[key] = accessibility * effort * detection
        return rates
    ...
    def expected_rate_array(
        ...
        *,
        ...
        accessibility = fields.accessibility[species]
        obs = {} if theta_obs is None else theta_obs
        covs = {} if covariates is None else covariates
        ...
        return ContextArray(
            accessibility.keys,
            accessibility.values * effort * detection,
        )
    ...
    def validate_species_data(self, species, data, keys) -> None:
        ...
        if any(value < 0 for value in values):
            raise ValueError("accessibility counts must be non-negative")
