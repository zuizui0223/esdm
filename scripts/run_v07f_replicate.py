#!/usr/bin/env python3
"""Run one frozen v0.7f out-of-family replicate."""
...
def _parser():
    parser = argparse.ArgumentParser(description="Run one frozen v0.7f replicate.")
...
def _seed(world: str, index: int) -> int:
    ...
    if name not in FROZEN_WORLDS:
        raise ValueError(f"unknown frozen v0.7f world {name!r}")
    value = int(index)
    if not 0 <= value < FROZEN_REPLICATES_PER_WORLD:
        raise ValueError("replicate outside frozen v0.7f range")
...
def main() -> int:
    from esdm.validate.v07f_run import run_v07f_replicate
