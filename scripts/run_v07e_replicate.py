#!/usr/bin/env python3
"""Run one frozen v0.7e reciprocal static-world replicate."""
...
FROZEN_BASE_SEED = 20261209
FROZEN_SEED_STRIDE = 167
FROZEN_WARMUP = 300
FROZEN_SAMPLES = 350
FROZEN_CHAINS = 2
...
def _parser():
    parser = argparse.ArgumentParser(description="Run one frozen v0.7e replicate.")
...
def _seed(index: int) -> int:
    ...
    if not 0 <= value < FROZEN_REPLICATES:
        raise ValueError("replicate outside frozen v0.7e range")
...
def main() -> int:
    from esdm.validate.v07e_run import run_v07e_replicate
