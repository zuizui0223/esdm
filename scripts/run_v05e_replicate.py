#!/usr/bin/env python3
"""Run one frozen v0.5e evidence-separation replicate."""
...
def _parser():
    parser = argparse.ArgumentParser(description="Run one frozen v0.5e replicate.")
...
def _seed(world: str, replicate: int) -> int:
    if world not in FROZEN_WORLD_OFFSETS:
        raise ValueError("unknown v0.5e world")
    index = int(replicate)
    if not 0 <= index < FROZEN_REPLICATES:
        raise ValueError("replicate outside frozen v0.5e range")
    return (
        FROZEN_BASE_SEED
        + FROZEN_WORLD_OFFSETS[world]
        + FROZEN_SEED_STRIDE * index
    )
...
def main() -> int:
    from esdm.validate.v05e_run import run_v05e_replicate
