#!/usr/bin/env python3
"""Aggregate frozen v0.5a directed interaction shards."""
...
FROZEN_BASE_SEED = 20261001
FROZEN_SEED_STRIDE = 73
FROZEN_NULL_OFFSET = 1000000


def _parser():
    parser = argparse.ArgumentParser(description="Aggregate frozen v0.5a shards.")
...
def _verify_gate():
    ...
    if observed != FROZEN_GATE_BLOB_SHA:
        raise RuntimeError("v0.5a gate blob mismatch")
    return observed
...
def main() -> int:
    ...
    from esdm.validate.v05a_run import V05AReplicate, summarize_v05a

    args = _parser().parse_args()
    output = Path(args.output)
    base = {
        "schema": "esdm.v05a.directed_known_truth.v1",
    ...
    try:
        ...
        if set(keyed) != expected:
            raise RuntimeError("v0.5a shard identities are incomplete or duplicated")
        ...
        for world, replicate in sorted(expected):
            ...
            records.append(V05AReplicate(**shard["record"]))

        qualification = evaluate_v05a_identification()
        summary = summarize_v05a(records)
        decision = evaluate_v05a_gate(qualification, summary)
