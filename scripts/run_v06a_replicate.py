#!/usr/bin/env python3
"""Run one frozen v0.6a accessibility recovery/transfer replicate."""
...
def _parser():
    parser = argparse.ArgumentParser(description="Run one frozen v0.6a replicate.")
...
def _seed(index: int) -> int:
    ...
    if not 0 <= value < FROZEN_REPLICATES:
        raise ValueError("replicate outside frozen v0.6a range")
...
def main() -> int:
    from esdm.validate.v06a_run import run_v06a_replicate
