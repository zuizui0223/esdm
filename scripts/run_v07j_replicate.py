#!/usr/bin/env python3
"""Run one frozen v0.7j population-shift replicate."""
...
FROZEN_GATE_COMMIT = "41d669b2b3fce602d969bd60b7c469de6e9ebc83"
FROZEN_GATE_BLOB_SHA = "7d576aef64fdc16373ec56eb60ba5a5c40bb6666"
GATE_PATH = (
...
def _parser():
    parser = argparse.ArgumentParser(description="Run one frozen v0.7j replicate.")
...
def _seed(world: str, index: int) -> int:
    ...
    if name not in FROZEN_WORLDS:
        raise ValueError(f"unknown frozen v0.7j world {name!r}")
    value = int(index)
    if not 0 <= value < FROZEN_REPLICATES_PER_WORLD:
        raise ValueError("replicate outside frozen v0.7j range")
    return FROZEN_BASE_SEEDS[name] + FROZEN_SEED_STRIDE * value
...
def main() -> int:
    from esdm.validate.v07j_run import run_v07j_replicate
