#!/usr/bin/env python3
"""Run one frozen v0.7j shifted-population replicate."""
...
def _parser():
    ...
    parser.add_argument("--world", choices=tuple(FROZEN_SEED_FAMILIES), required=True)
...
def _verify_gate():
    ...
    if observed != FROZEN_GATE_BLOB_SHA:
        raise RuntimeError(
            f"v0.7j gate blob mismatch: expected {FROZEN_GATE_BLOB_SHA}, "
            f"observed {observed}"
        )
    return observed
...
def _seed(world: str, replicate: int) -> int:
    if world not in FROZEN_SEED_FAMILIES:
        raise KeyError(f"unknown v0.7j world {world!r}")
    value = int(replicate)
