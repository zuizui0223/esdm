#!/usr/bin/env python3
"""Run the frozen pre-MCMC v0.6a accessibility qualification."""
...
def _parser():
    parser = argparse.ArgumentParser(description="Run frozen v0.6a qualification.")
...
def _verify_gate():
    ...
    if observed != FROZEN_GATE_BLOB_SHA:
        raise RuntimeError(
            f"v0.6a gate blob mismatch: expected {FROZEN_GATE_BLOB_SHA}, "
            f"observed {observed}"
        )
    return observed
...
def main() -> int:
    ...
    base = {
        "schema": "esdm.v06a.qualification.v1",
