#!/usr/bin/env python3
"""Run frozen pre-MCMC v0.7f candidate qualification."""
...
def _parser():
    parser = argparse.ArgumentParser(description="Run frozen v0.7f qualification.")
...
def _verify_gate():
    ...
    if observed != FROZEN_GATE_BLOB_SHA:
        raise RuntimeError(
            f"v0.7f gate blob mismatch: expected {FROZEN_GATE_BLOB_SHA}, "
            f"observed {observed}"
        )
    return observed
...
def main() -> int:
    ...
    base = {
        "schema": "esdm.v07f.qualification.v1",
