#!/usr/bin/env python3
"""Run one frozen v0.7m absolute-adequacy policy replicate."""
...
def _parser():
    parser = argparse.ArgumentParser(description="Run one frozen v0.7m replicate.")
...
def _seeds(world: str, index: int):
    ...
    if name not in FROZEN_WORLDS:
        raise ValueError(f"unknown frozen v0.7m world {name!r}")
    value = int(index)
    if not 0 <= value < FROZEN_REPLICATES_PER_WORLD:
        raise ValueError("replicate outside frozen v0.7m range")
    ...
    if pilot == confirm:
        raise RuntimeError("v0.7m pilot and confirmatory seeds collided")
...
def main() -> int:
    from esdm.validate.v07m_run import run_v07m_replicate
