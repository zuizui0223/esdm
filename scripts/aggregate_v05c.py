#!/usr/bin/env python3
"""Aggregate frozen v0.5c event-gated claim shards."""
...
def _parser():
    parser = argparse.ArgumentParser(description="Aggregate frozen v0.5c shards.")
...
def _verify_gate():
    ...
    if observed != FROZEN_GATE_BLOB_SHA:
        raise RuntimeError("v0.5c gate blob mismatch")
    return observed
...
def main() -> int:
    ...
    from esdm.validate.v05c_run import V05CReplicate, summarize_v05c

    args = _parser().parse_args()
    output = Path(args.output)
    base = {
        "schema": "esdm.v05c.event_gated_claim.v1",
    ...
    try:
        ...
        if set(keyed) != expected:
            raise RuntimeError(
                "v0.5c shard identities are incomplete or duplicated"
            )
        ...
        for world, replicate in sorted(expected):
            ...
            if shard.get("status") != "COMPLETE":
                raise RuntimeError(
                    f"v0.5c shard {world}/{replicate} not COMPLETE"
                )
            if shard.get("gate_freeze_commit") != FROZEN_GATE_COMMIT:
                raise RuntimeError("v0.5c shard gate commit mismatch")
            if shard.get("gate_blob_sha") != FROZEN_GATE_BLOB_SHA:
                raise RuntimeError("v0.5c shard gate blob mismatch")
            if int(shard["seed"]) != _seed(world, replicate):
                raise RuntimeError(
                    f"v0.5c shard seed mismatch: {world}/{replicate}"
                )
            records.append(V05CReplicate(**shard["record"]))

        summary = summarize_v05c(records)
        decision = evaluate_v05c_gate(summary)
