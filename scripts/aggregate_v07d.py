#!/usr/bin/env python3
"""Aggregate frozen v0.7d static-versus-dynamic shards."""
...
FROZEN_GATE_COMMIT = "c50e05f049da05c5b5063d5034cacfae9e8ecd98"
FROZEN_GATE_BLOB_SHA = "131f0376a5097aa97358ff2511a63c68f2922a53"
GATE_PATH = (
    Path(__file__).resolve().parents[1]
    / "docs" / "validation" / "V07D_EQUAL_DIMENSION_GATE.md"
)
FROZEN_REPLICATES = 16
FROZEN_BASE_SEED = 20261201
FROZEN_SEED_STRIDE = 163


def _parser():
    parser = argparse.ArgumentParser(description="Aggregate frozen v0.7d shards.")
...
def _verify_gate():
    ...
    if observed != FROZEN_GATE_BLOB_SHA:
        raise RuntimeError("v0.7d gate blob mismatch")
    return observed
...
def main() -> int:
    ...
    from esdm.validate.v07d_run import V07DReplicate, summarize_v07d

    args = _parser().parse_args()
    output = Path(args.output)
    base = {
        "schema": "esdm.v07d.static_dynamic.v1",
    ...
    try:
        ...
        if set(keyed) != set(range(FROZEN_REPLICATES)):
            raise RuntimeError(
                "v0.7d shard identities are incomplete or duplicated"
            )
        ...
        for replicate in range(FROZEN_REPLICATES):
            ...
            records.append(V07DReplicate(**shard["record"]))

        qualification = evaluate_v07d_identification()
        summary = summarize_v07d(records)
        decision = evaluate_v07d_gate(qualification, summary)
