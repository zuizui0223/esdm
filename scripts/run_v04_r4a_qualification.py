#!/usr/bin/env python3
"""Run the frozen v0.4-R4a identification-only qualification gate."""
...
FROZEN_GATE_COMMIT = "65115f31425d4d8655137eeea9a0bdb40f56aa53"
FROZEN_GATE_BLOB_SHA = "c55af9dfbc6347218fecebb27542ee7bb59310b0"
GATE_PATH = (
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Frozen v0.4-R4a identification-only qualification. "
            "The 36 x 12 phase-balanced design and all thresholds are not configurable."
        )
    )
    parser.add_argument(
        "--output",
        default="v04_r4a_qualification.json",
...
def _verify_gate_blob() -> str:
    if not GATE_PATH.exists():
        raise RuntimeError(f"frozen R4a gate file is missing: {GATE_PATH}")
    observed = _git_blob_sha1(GATE_PATH.read_bytes())
    if observed != FROZEN_GATE_BLOB_SHA:
        raise RuntimeError(
            "R4a gate blob mismatch: "
...
def _fetch_source() -> tuple[bytes, dict[str, str]]:
    request = urllib.request.Request(
        FROZEN_SOURCE_URL,
        headers={"User-Agent": "esdm-v04-r4a-qualification"},
    )
...
def _initial_payload() -> dict:
    return {
        "schema": "esdm.v04_r4a.qualification.v1",
        "git_sha": _git_sha(),
        ...
        "infrastructure_block": {
            ...
            "reason": "R4a qualification has not completed",
        },
        "frozen_design": {
...
def main() -> int:
    ...
    from esdm.validate.v04_r4a_gate import (
        evaluate_v04_r4a_identification,
        evaluate_v04_r4a_qualification,
    ...
    try:
        ...
        identification = evaluate_v04_r4a_identification(source_text)
        summary = qualification_summary(source_text, identification)
        decision = evaluate_v04_r4a_qualification(summary)
        fixture = build_v04_r4a_fixture(source_text)
