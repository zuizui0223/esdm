#!/usr/bin/env python3
"""Aggregate frozen v0.7g calibration-placement shards."""
...
FROZEN_GATE_COMMIT = "5efbb6a34ab202c702d1206de325d3781185204a"
FROZEN_GATE_BLOB_SHA = "a37866fb2bd368515f697aa5dbd3f3fb9da04ca8"
GATE_PATH = (
    Path(__file__).resolve().parents[1]
    / "docs" / "validation" / "V07G_CALIBRATION_PLACEMENT_GATE.md"
)
FROZEN_REPLICATES = 16
FROZEN_BASE_SEED = 20261225
FROZEN_SEED_STRIDE = 179


def _parser():
    parser = argparse.ArgumentParser(description="Aggregate frozen v0.7g shards.")
...
def _verify_gate():
    ...
    if observed != FROZEN_GATE_BLOB_SHA:
        raise RuntimeError("v0.7g gate blob mismatch")
    return observed
...
def main() -> int:
    ...
    from esdm.validate.v07g_run import V07GReplicate, summarize_v07g

    args = _parser().parse_args()
    output = Path(args.output)
    base = {
        "schema": "esdm.v07g.calibration_placement.v1",
    ...
    try:
        ...
        if set(keyed) != set(range(FROZEN_REPLICATES)):
            raise RuntimeError(
                "v0.7g shard identities are incomplete or duplicated"
            )
        ...
        for replicate in range(FROZEN_REPLICATES):
            ...
            records.append(V07GReplicate(**shard["record"]))

        summary = summarize_v07g(records)
        decision = evaluate_v07g_gate(summary)
